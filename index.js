...
// Route to serve an HTML page for file upload
app.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, 'index.html'));
});

// Route to handle file uploads and send the file to Flask
app.post('/upload-csv', upload.single('csvFile'), async (req, res) => {
    ...
    try {
        ...
        const flaskResponse = await axios.post(
            'https://caschooldistrictsfinancialdataexplorer.onrender.com/update-csv',
            formData,
            {
                headers: {
                    ...formData.getHeaders(),
        ...
        // // Cleanup: Delete the uploaded file after sending
        // fs.unlinkSync(req.file.path);

        res.json({
            message: 'CSV uploaded and sent to Flask successfully',
        ...
        res.status(500).send('Error sending file to Flask');
    }
});
